...
byte ADConverter::ReallyChannel(ADC_Channel ParameterChannel)
{
	if(ParameterChannel < PA10)return (byte)ParameterChannel;
	if(ParameterChannel < PC6)
	{
		int temp = (int)ParameterChannel ;
		temp &= 0x0f;
		temp += 0x0a;
		return temp;
	}
	if(ParameterChannel == Vrefint)return 0x10;
	if(ParameterChannel == Vrefint)return 0x11;
}
...
ADConverter:: ADConverter(ADC_Channel line)
{
	...
	{
		adc_IntStrs[_ADC_group] = new ADC_InitTypeDef();	// 申请一个ADC配置类结构体class   初始化参数由构造函数完成
		ADC_InitTypeDef * adc_intstr=adc_IntStrs[_ADC_group];	// 取得对象
