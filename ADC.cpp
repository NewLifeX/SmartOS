...
static ADC_TypeDef * const g_ADCs[3]= {ADC1,ADC2,ADC3};


int isSmall()
{
	if(_ADC_ChanelNum[1] <= _ADC_ChanelNum[2])
	{
		if(_ADC_ChanelNum[0] <= _ADC_ChanelNum[1])return 0;
		else  return 1;
	}
	else
	{
		if(_ADC_ChanelNum[0] <= _ADC_ChanelNum[2])return 0;
		else  return 2;
	}
}
...
ADConverter:: ADConverter(ADC_Channel line)
{
	...
	int isLittleLine = isSmall();
	if(_ADC_ChanelNum[isLittleLine] == 0)
	{
		ADC_InitTypeDef adc_intstr;	// 初始化ADc使用的结构体
	...
	/*	ADC_NbrOfChannel:
		开启通道数   */
		adc_intstr.ADC_NbrOfChannel=1;									//蛋疼的参数	！！！！
