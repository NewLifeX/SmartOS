...
static TokenController* AddMaster(LinkClient& client)
{
...
static TokenController* AddControl(LinkClient& client, NetworkInterface* host, const NetUri& uri, ushort remotePort)
{
	// 创建连接服务器的Socket
...
void LinkClient::CheckNet()
{
	...
	if (!linked && mst)
	{
		Delegate2<TokenMessage&, TokenController&> dlg(&LinkClient::OnReceive, this);
		mst->Received = dlg;
		mst->Open();
	...
	if (_LocalReceive)
	{
		...
		for (int k = 0; k < nis.Count(); k++)
		{
			...
			if (!flag)
			{
				debug_printf("LinkClient::CheckNet %s 创建本地监听 ", nis[k]->Name);
...
void LinkClient::UseLocal()
{
	_LocalReceive = Delegate2<TokenMessage&, TokenController&>(&LinkClient::OnReceiveLocal, this);
}

bool LinkClient::Send(TokenMessage& msg, TokenController* ctrl)
{
	// 未登录之前，只能 握手、登录、注册
	if (Token == 0)
	{
...
bool LinkClient::Reply(TokenMessage& msg, TokenController* ctrl)
{
	// 未登录之前，只能 握手、登录、注册
	if (ctrl->Token == 0)
	{
...
void LinkClient::OnReceive(TokenMessage& msg, TokenController& ctrl)
{
...
void LinkClient::OnReceiveLocal(TokenMessage& msg, TokenController& ctrl)
{
	TS("LinkClient::OnReceiveLocal");
...
// 内网分发
void LinkClient::LocalSend(int start, const Buffer& bs)
{
	auto& cs = Sessions;
...
void LinkClient::SayHello(bool broadcast)
{
	TS("LinkClient::SayHello");
...
// 握手响应
bool LinkClient::OnHello(TokenMessage& msg, TokenController* ctrl)
{
...
bool LinkClient::OnRedirect(HelloMessage& msg)
{
	TS("LinkClient::OnRedirect");
...
bool LinkClient::ChangeIPEndPoint(const NetUri& uri)
{
	TS("LinkClient::ChangeIPEndPoint");
...
// 注册
void LinkClient::Register()
{
	TS("LinkClient::Register");

	debug_printf("LinkClient::Register\r\n");
...
void LinkClient::OnRegister(TokenMessage& msg, TokenController* ctrl)
{
	if (!msg.Reply) return;

	TS("LinkClient::OnRegister");
...
bool LinkClient::OnLogin(TokenMessage& msg, TokenController* ctrl)
{
	if (!msg.Reply) return false;
...
bool LinkClient::OnPing(TokenMessage& msg, TokenController* ctrl)
{
...
void LinkClient::Read(int start, int size)
{
	TokenDataMessage dm;
	dm.Start = start;
	dm.Size = size;
...
// 异步上传并等待响应
int LinkClient::WriteAsync(int start, const Buffer& bs, int msTimeout)
{
	// 如果别的任务正在上传，此次写入失败
	if (_Expect) return 0;
...
响应：起始 + 数据
*/
void LinkClient::OnRead(const TokenMessage& msg, TokenController* ctrl)
{
	if (msg.Reply) return;
...
错误：错误码2 + 1起始 + 1大小
*/
void LinkClient::OnWrite(const TokenMessage& msg, TokenController* ctrl)
{
	if (msg.Length < 2) return;
...
void LinkClient::Invoke(const String& action, const Buffer& bs)
{
	TokenMessage msg(8);
...
void LinkClient::OnInvoke(const TokenMessage& msg, TokenController* ctrl)
{
	auto rs = msg.CreateReply();
...
bool LinkClient::OnInvoke(const String& action, const Pair& args, Stream& result)
{
...
void LinkClient::Register(cstring action, InvokeHandler handler, void* param)
{
	if (handler)
	{
...
bool LinkClient::InvokeRestart(void * param, const Pair& args, Stream& result)
{
...
bool LinkClient::InvokeReset(void * param, const Pair& args, Stream& result)
{
...
bool LinkClient::InvokeSetRemote(void * param, const Pair& args, Stream& result)
{
	BinaryPair res(result);
...
bool LinkClient::InvokeGetRemote(void * param, const Pair& args, Stream& result)
{
	debug_printf("获取远程地址\r\n");
	auto client = (LinkClient*)param;
...
bool LinkClient::InvokeGetAllApi(void * param, const Pair& args, Stream& result)
{
	debug_printf("获取Apis\r\n");
	auto client = (LinkClient*)param;
