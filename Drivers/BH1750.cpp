...
BH1750::BH1750(I2C_TypeDef* iic, uint speedHz )
{
	assert_param(iic);
...
void BH1750::SetPin(Pin acl , Pin sda )
{
	Pins[0] = acl;
	Pins[1] = sda;
}

void BH1750::GetPin(Pin* acl , Pin* sda )
{
	*acl = Pins[0];
	*sda = Pins[1];
}
...
bool BH1750::WaitForEvent(uint event)
{
	int retry = Retry;
...
bool BH1750::SetID(byte id, bool tx)
{
	// 产生起始条件
...
void BH1750::Write(byte id, byte addr, byte dat)
{
	if(!SetID(id)) return;
...
byte BH1750::Read(byte id, byte addr)
{
	// 等待I2C
